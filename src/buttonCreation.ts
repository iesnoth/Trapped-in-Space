...
function newButton(url:string, id:string, id2:string):unknown {
    ...
    document.getElementById(id:string).appendChild(button);
    return button
}
...
function buttonFunction(url1: string, url2:string, id:string, id2:string):unknown {
    let button = newButton(url1, id, id2)
    //turns buttons off and on, adds function
    button.addEventListener('click', (e:null) => {
...
function throttleFunction(url1:string, url2:string, id:string, id2:string) {
    let button = newButton(url1, id, id2)
...
function move(element:unknown):unknown {
